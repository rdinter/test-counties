...
Data have yet to be parsed for 1990 and 1991 because the `.txt` format is not easily readable. Further, because of data concerns with a large number of negative income flows of extremely unlikely negative values for income flows, the 1993 and 1994 years are omitted:

```{r Example Negatives}
allindata %>% filter(Aggr_AGI < 0 & !is.na(Aggr_AGI)) %>% arrange(Aggr_AGI) %>% select(State_Abbrv:year) %>%  head(., n = 5) %>% kable(., caption = "Inflows")

alloutdata %>% filter(Aggr_AGI < 0 & !is.na(Aggr_AGI)) %>% arrange(Aggr_AGI) %>% select(State_Abbrv:year) %>% head(., n = 5) %>% kable(., caption = "Outflows")
```

*where* `Return_num` *indicates aggregate tax returns between the origin county (ofips) and destination county (dfips) to proxy for number of households;* `Exmpt-num` *refers to the number of exemptions filed which proxies population;* `Aggr_AGI` *is the sum of Adjusted Gross Income for all filers and is in units of \$1,000s.*
...
The flagged values appear to be minimal. If using `AGI`, then it is best to remove these flagged values. However, the `Return_Num` and `Exmpt_Num` do not appear to suffer. Using this data for population migration would be considered appropriate. Correcting the `AGI` problem is left for future work.
...
```{r Suppressed}
kable(allindata %>% group_by(year) %>% summarise(Suppressed = sum(Return_Num == -1 | is.na(Return_Num)), Total = n(), Pct_Suppressed = round(100*Suppressed / Total, digits = 2), Minimum = sum(Return_Num == 10 & !is.na(Return_Num)), Pct_Minimum = round(100*Minimum / Total, digits = 2)))
```
...
Working backwards a bit, in the 2011 and 2012 documentation we have the following special codes for each county:
...
| County Total Migration Flows:      | State FIPS | County FIPS | Pre-1995 Change |
|:-----------------------------------|------------|-------------|-------------:|
| **Non-Migrant (identical FIPS)**   | **NA**     | **NA**      | **ST `63` + `50` to ST \& CTY of interest** |
|**Total Migration – US and Foreign**| **96**     | **000**     | **ST `0` + CTY `1` to `96` + `0`** |
| **Total Migration – Foreign**      | **98**     | **000**     | **NewVariable: SUM all `57` flows** |
...
| **Total Migration – US**          | **97**     | **000**     | **New variable (for pre-2004): `97` + `0` = `96000` minus `98000`** |
...
| Total Migration – Same State      | 97         | 001         | New Variable: Sum across ST `XX` less its own + `58000` |
| Total Migration – Different State | 97         | 003         | New Variable: `97000` minus `97001` |

For the purposes of this project, I do not have a substantial reason to have observations based upon different states or regions. The `97001`, `97003`, and all ST `58` and `59` are not important to me.

## State Level Migration
Starting in 1995, the IRS data also provides summaries for the flows between a state and a county with a given `000` for County Code:

```{r State Aggregate}
allindata %>% filter(County_Code_Origin == 0) %>% group_by(year) %>% summarise(total = n()) %>% kable
```

Because of this, one needs to take care in making their own summary statistics in a county-to-county dataset. These values will be removed because the focus is on county-to-county only.

# Inflow versus Outflow Merge
The IRS data contains two sets of files: one for the inflow between regions and one for the outflow between regions. I have looked across the IRS documentation in attempt to verify that the inflow from region A to region B should also be the outflow from region B to region A. I have not found confirmation of this from documentation, so we need to attempt to figure this out through inspection of the data. A casual glance indicates to me that 2004 appears to be the first year that the IRS matches up county-to-county connections across inflows and outflows.
